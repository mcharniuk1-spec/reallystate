...
- **Apartment image-description reports** after Codex confirms complete local-gallery rows. Use `docs/exports/taskforgema.md` and only local image files listed in `local_image_files`.
...
- Do not run or widen tier-1/2 scraping by yourself unless the operator explicitly asks for a live scrape. For the next planned Gemma pass, consume the Codex-prepared eligible listing set and generate image reports only.
...
- For image-report work: the exact eligible listing JSON files, local image file list, and output directory under `docs/exports/apartment-image-reports/`.
...
2. Gemma4 runs `S1-22`: creates per-apartment image reports from the Codex-confirmed local galleries.
