...
def _serialize_property(m: Any) -> dict[str, Any]:
    return {
        ...
        "canonical_url": getattr(m, "canonical_url", None),
        "canonical_address": m.canonical_address,
        "canonical_city": m.canonical_city,
        "canonical_building_name": m.canonical_building_name,
        "source_links": list(getattr(m, "source_links", []) or []),
        "merged_image_urls": list(getattr(m, "merged_image_urls", []) or []),
        "description_summary": getattr(m, "description_summary", None),
        "latitude": m.latitude,
